    ...
    <div style={s.app}>
      ...
      <div style={{ ...s.content, padding: isMobile ? "16px 12px" : "24px" }}>
        <div style={{ ...s.headerRow, flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "flex-end" }}>
          ...
          <div style={s.controls}>
            {activeTab === "calc" && (
              <button style={{ ...s.syncBtn, background: selectedPays.length > 0 ? "var(--color-green)" : "var(--color-border)", color: selectedPays.length > 0 ? "#fff" : "#9ca3af" }} disabled={selectedPays.length === 0} onClick={handlePayLocal}>
            ...
            <button 
              ...
              style={{
                background: onlyActive ? "var(--color-accent-soft)" : "var(--color-card)",
                border: `1px solid ${onlyActive ? "var(--color-accent)" : "var(--color-border)"}`,
                padding: "8px 12px", borderRadius: 8, fontSize: 12, fontWeight: 700,
                color: onlyActive ? "var(--color-accent)" : "var(--color-text-2)", cursor: "pointer"
              }}
            >
        ...
        {activeTab === "schedule" && (
          <div style={s.tableWrap} ref={scheduleScrollRef}>
            <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", background: "var(--color-card)", borderBottom: "1px solid var(--color-border)", gap: 12, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 11, color: "var(--color-text-3)", fontWeight: 600 }}>Сортировка:</span>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: sortMode === mode ? "var(--color-accent)" : "var(--color-card)",
                      color: sortMode === mode ? "#fff" : "var(--color-text-2)",
                      borderColor: sortMode === mode ? "var(--color-accent)" : "var(--color-border)",
            ...
            <table style={s.table}>
              <thead>
                <tr>
                  <th style={{ ...s.th, width: 220, position: "sticky", left: 0, top: 0, zIndex: 20, background: "var(--color-surface)", boxShadow: "2px 2px 5px -2px rgba(0,0,0,0.1)" }}>Курьер</th>
                  <th style={{ ...s.th, width: 120, position: "sticky", top: 0, zIndex: 10, background: "var(--color-surface)", boxShadow: "0 2px 5px -2px rgba(0,0,0,0.1)" }}>Телефон</th>
                  {scheduleDates.map((d) => {
                    ...
                    return (
                      <th key={d} id={`day-${d}`} style={{ ...s.th, textAlign: "center", cursor: "pointer", color: sortDate === d ? "var(--color-accent)" : "var(--color-text-3)", background: sortDate === d ? "var(--color-accent-soft)" : "var(--color-surface)", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 2px 5px -2px rgba(0,0,0,0.1)" }} onClick={() => setSortDate(d)}>
                        {dayStr}
                      </th>
                    );
                  })}
              ...
              <tbody>
                {loading ? <tr><td colSpan={9} style={{ padding: 20, textAlign: "center" }}>Загрузка...</td></tr> : scheduleSorted.map(c => {
                  ...
                  return (
                    <tr key={c.id} style={{ background: isSortDayWorking ? "var(--color-surface)" : "var(--color-card)", borderBottom: "1px solid #f0efe9" }}>
                      <td style={{ ...s.td, fontWeight: 600, position: "sticky", left: 0, zIndex: 5, background: isSortDayWorking ? "var(--color-surface)" : "var(--color-card)", boxShadow: "2px 0 5px -2px rgba(0,0,0,0.1)" }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', maxWidth: '190px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', gap: '4px' }} onClick={(e) => { e.stopPropagation(); toggleAuto(c.id, c.isAuto || false); }}>
                              ...
                              <div style={{ position: 'relative', width: 28, height: 16, background: c.isAuto ? 'var(--color-green)' : 'var(--color-border)', borderRadius: 20, transition: '0.2s', flexShrink: 0 }}>
                      ...
                      <td style={{ ...s.td, color: "var(--color-text-2)", fontSize: 12, verticalAlign: "middle" }}>{c.phone || "—"}</td>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                              ...
                              {isWorking && (
                                <div style={{ display: 'flex', gap: 4, alignItems: 'center', justifyContent: 'center', marginTop: 4, background: "var(--color-card)", padding: "4px 6px", borderRadius: 6, border: "1px solid var(--color-border)" }}>
                                  ...
                                  <span style={{ fontSize: 11, color: "var(--color-text-3)", fontWeight: 700 }}>-</span>
                                  <select value={shift.endTime || "22:00"} onChange={e => updateShiftDetails(c.id, date, { endTime: e.target.value })} style={{ fontSize: 11, padding: "2px 0", width: 44, border: "none", outline: "none", cursor: "pointer", background: "transparent", textAlign: "center", fontWeight: 600, color: "var(--color-text)", appearance: "none" }}>
                                    {TIME_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
                                  </select>
                                </div>
                              )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "var(--color-card)", borderRadius: 12, border: "1px solid var(--color-border)", boxShadow: "0 2px 8px rgba(0,0,0,0.02)", flexWrap: "wrap", gap: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <button onClick={() => checkKonsolStatuses(false)} disabled={konsolLoading} style={{ ...s.navBtn, background: "var(--color-accent-soft)", color: "var(--color-accent-fg)", borderColor: "var(--color-accent)" }}>
                  🔄 Статусы
                </button>
                <button onClick={createSelectedTask} disabled={konsolLoading || selectedPays.length === 0} style={{ background: konsolLoading || selectedPays.length === 0 ? "var(--color-border)" : "var(--color-card)", color: konsolLoading || selectedPays.length === 0 ? "#9ca3af" : "var(--color-text)", border: "1px solid var(--color-border)", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: konsolLoading || selectedPays.length === 0 ? "not-allowed" : "pointer" }}>
                  ➕ Создать задание
                </button>
                <button onClick={handleRecalculate} disabled={loading || selectedPays.length === 0} style={{ background: loading || selectedPays.length === 0 ? "var(--color-border)" : "var(--color-accent)", color: loading || selectedPays.length === 0 ? "#9ca3af" : "#fff", border: "none", padding: "10px 16px", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: loading || selectedPays.length === 0 ? "not-allowed" : "pointer", }}>
                  {loading ? "⏳ Загрузка..." : "🔄 Пересчитать"}
                </button>
                <button onClick={handleFinalize} disabled={loading || selectedPays.length === 0} style={{ background: loading || selectedPays.length === 0 ? "var(--color-border)" : "#f59e0b", color: loading || selectedPays.length === 0 ? "#9ca3af" : "#fff", border: "none", padding: "10px 16px", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: loading || selectedPays.length === 0 ? "not-allowed" : "pointer", }}>
                  {loading ? "⏳ Загрузка..." : "📝Закрыть и Акт"}
                </button>
                <button onClick={handlePay} disabled={loading || selectedPays.length === 0} style={{ background: loading || selectedPays.length === 0 ? "var(--color-border)" : "var(--color-green)", color: loading || selectedPays.length === 0 ? "#9ca3af" : "#fff", border: "none", padding: "10px 16px", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: loading || selectedPays.length === 0 ? "not-allowed" : "pointer", }}>
            ...
            <div style={s.tableWrap}>
              ...
              <thead>
                <tr>
                  <th style={{ ...s.th, width: 220, verticalAlign: "top", position: "sticky", left: 0, top: 0, zIndex: 20, background: "var(--color-surface)", boxShadow: "2px 2px 5px -2px rgba(0,0,0,0.1)" }}>
                  ...
                  {calcDates.map(d => {
                    ...
                    const dayStr = `${dObj.toLocaleDateString('ru', { weekday: 'short' }).toUpperCase()} ${dObj.toLocaleDateString('ru', { day: '2-digit', month: '2-digit' })}`;
                    return (
                      <th key={d} style={{ ...s.th, textAlign: "center", verticalAlign: "top", whiteSpace: "nowrap", position: "sticky", top: 0, zIndex: 10, background: "var(--color-surface)", boxShadow: "0 2px 5px -2px rgba(0,0,0,0.1)" }}>
                        {dayStr}
                        <div style={{ marginTop: 6, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                          ...
                          {daySelectedSum > 0 && <span style={{ fontSize: 10, color: "var(--color-accent-fg)", fontWeight: 700 }}>{daySelectedSum} ₽</span>}
                        </div>
                      </th>
                    )
                  })}
              ...
              <tbody>
                {loading ? <tr><td colSpan={9} style={{ padding: 20, textAlign: "center" }}>Загрузка...</td></tr>
                  : calcSortedAndFiltered.length === 0 ? <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: "var(--color-text-3)" }}>На этой неделе нет курьеров с заказами</td></tr>
                      ...
                      return (
                        <tr key={c.id} style={{ borderBottom: "1px solid #f0efe9", background: "var(--color-card)" }}>
                          ...
                          {calcDates.map(d => {
                            ...
                            return (
                              <td key={d} style={{ ...s.td, textAlign: "center", verticalAlign: "top", background: isPaid ? "var(--color-ok-bg)" : "transparent" }}>
                                {(count > 0 || sum > 0) ? (
                                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                                    ...
                                    <div style={{ fontSize: 11, color: "var(--color-accent-fg)", fontWeight: 700 }}>{sum} ₽</div>
                                  </div>
                                ) : <span style={{ color: "#d1d5db" }}>—</span>}
                              </td>
                            );
                          })}
                          <td style={{ ...s.td, textAlign: "right", fontWeight: 700, background: "var(--color-surface)" }}>
              ...
              return (
                <div key={c.id} style={{ background: "var(--color-card)", borderRadius: 12, border: "1px solid var(--color-border)", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.02)" }}>
                  <div onClick={() => setExpandedCouriers(prev => ({ ...prev, [c.id]: !isCExpanded }))} style={{ padding: "14px 16px", background: "var(--color-surface)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: isCExpanded ? "1px solid var(--color-border)" : "none" }}>
                    ...
                    <div style={{ fontSize: 18, color: "var(--color-text-3)", transform: isCExpanded ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>▼</div>
                  </div>
                  ...
                  {isCExpanded && (
                    <div style={{ padding: isMobile ? 12 : 16, display: "flex", flexDirection: "column", gap: 20 }}>
                      ...
                      {courierUnassignedOrders.length > 0 && (
                        <div>
                          <h4 style={{ margin: "0 0 12px 0", fontSize: 14, fontWeight: 700, color: "var(--color-text-2)" }}>Без маршрута ({courierUnassignedOrders.length})</h4>
                              ...
                              return (
                                <div key={o.id} style={{ background: "var(--color-surface)", borderRadius: 10, border: "1px dashed var(--color-text-3)", padding: 14, display: "flex", flexDirection: "column" }}>
                                  ...
                                  <div style={{ display: "flex", gap: 8 }}>
                                    ...
                                    <button onClick={() => createRouteFromUnassigned(o.id, c.id)} style={{ flex: 2, background: "rgba(74, 122, 255, 0.08)", color: "var(--color-accent-fg)", border: "none", padding: "6px 0", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>➕ В новый маршрут</button>
                                  </div>
                                </div>
                              );
                            })}
      ...
      {activeTab === "tasks" && (() => {
        const DB_ST: Record<string, { label: string; color: string; bg: string }> = {
          ...
          CONFIRMED: { label: "🔵 Принято", color: "var(--color-accent-fg)", bg: "var(--color-accent-soft)" },
          ACCEPTED: { label: "🟢 Выполнено", color: "var(--color-green)", bg: "var(--color-ok-bg)" },
        ...
        const REMOTE_ST: Record<string, { label: string; color: string }> = {
          ...
          confirmed: { label: "🔵 В работе", color: "var(--color-accent-fg)" },
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "var(--color-card)", borderRadius: 10, border: "1px solid var(--color-border)", flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginRight: 16 }}>
                <button style={s.arrowBtn} onClick={() => setWeekStart(d => new Date(d.getTime() - 7 * 86400000))}>◀ Неделя</button>
                <span style={{ fontWeight: 700, fontSize: 13, textTransform: "capitalize", color: "var(--color-text)" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: konsolTasksLoading ? "var(--color-bg)" : "var(--color-card)",
                  minWidth: 120,
                }}
              >
                {konsolTasksLoading ? (
              ...
              {selectedTasks.size > 0 && <>
                ...
                {selCanFinalize > 0 && <button onClick={() => handleTasksAction("finalize")} disabled={konsolTasksLoading} style={{ ...s.navBtn, background: "var(--color-accent-soft)", color: "var(--color-accent-fg)", borderColor: "var(--color-accent)" }}>📄 Финализ. ({selCanFinalize})</button>}
            ...
            {konsolTasksLoading && konsolTasks.length === 0 ? (
              <div style={{ padding: 60, textAlign: "center", color: "var(--color-text-3)" }}>Загрузка...</div>
            ) : konsolTasks.length === 0 ? (
              <div style={{ padding: 60, textAlign: "center", color: "var(--color-text-3)" }}>Нет заданий на этой неделе.</div>
              ...
              return (
                <div key={courierId} style={{ background: "var(--color-card)", borderRadius: 12, border: "1px solid var(--color-border)", overflow: "hidden" }}>
                  ...
                  {(cTasks as any[]).map((task: any, idx: number) => {
                    ...
                    const remoteSt = remote?.state ? (REMOTE_ST[remote.state.code] || { label: remote.state.title, color: "var(--color-text-2)" }) : null;
                      ...
                      <div key={task.id}>
                        <div
                          style={{
                            ...
                            background: isSelected ? "var(--color-accent-soft)" : isPaid ? "var(--color-ok-bg)" : idx % 2 === 1 ? "var(--color-surface)" : "var(--color-card)",
                        ...
                        >
                          ...
                          <a href={`https://app.konsol.pro/tasks/${task.konsolTaskId}`} target="_blank" rel="noopener noreferrer"
                            ...
                            style={{ fontFamily: "monospace", fontSize: 11, color: "var(--color-accent-fg)", textDecoration: "none" }}>
                            #{task.konsolTaskId}
                          </a>
                          ...
                          {remoteSt ? (
                            ...
                            <span style={{ fontSize: 11, background: "var(--color-bg)", color: "var(--color-text-3)", padding: "2px 8px", borderRadius: 20, fontWeight: 700 }}>⏳ загрузка...</span>
                          ) : (
                          ...
                          <div onClick={e => e.stopPropagation()} style={{ display: "flex", gap: 4 }}>
                            {(!isPaid && !task.konsolActId) && (
                              <button
                                ...
                                disabled={konsolTasksLoading}
                                style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--color-accent-fg)", background: "var(--color-accent-soft)", color: "var(--color-accent-fg)", fontSize: 12, cursor: "pointer" }}
                          ...
                          <span style={{ fontSize: 10, color: "var(--color-text-3)", transform: isExpanded ? "rotate(180deg)" : "rotate(0)", display: "inline-block", transition: "0.2s" }}>▼</span>
                        </div>
                        ...
                        {isExpanded && (
                          <div style={{ padding: "12px 20px 12px 48px", background: "#f8f7ff", borderBottom: "1px solid var(--color-border)" }}>
                            {remote === null ? (
                              ...
                              <div style={{ color: "var(--color-text-3)", fontSize: 12 }}>Услуги не найдены</div>
                            )}
                          </div>
                        )}
                      </div>
