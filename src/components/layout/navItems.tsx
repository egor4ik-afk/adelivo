...
export const NAV_ITEMS: NavItem[] = [
  { href: "/dashboard", label: "Дашборд", icon: "🗺", hint: "Карта и распределение заказов", roles: ["ADMIN", "OPERATOR"] },
  { href: "/manager", label: "Менеджер", icon: "📋", hint: "Маршруты и изменения", roles: ["ADMIN", "OPERATOR"] },
  { href: "/couriers", label: "Курьеры", icon: "🚚", hint: "Смены, выплаты, геолокация", roles: ["ADMIN", "OPERATOR"] },
  ...
  { href: "/orders", label: "Заказы", icon: "≡", hint: "Все заказы списком", roles: ["ADMIN", "OPERATOR"], secondary: true },
...
export function navFor(role: string, isSuperAdmin = false): NavItem[] {
  return NAV_ITEMS.filter(
    (n) => (n.roles.includes(role) || isSuperAdmin) && (!n.superOnly || isSuperAdmin)
  );
}
