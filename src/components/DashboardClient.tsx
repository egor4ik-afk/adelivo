            ...
            {!selected ? (
              ...
              <div style={s.detailScroll}>
                ...
                <div style={s.fieldsGrid}>
                  ...
                  <DetailField label="Курьер" value={selected.courier ?? "Не назначен"} highlight={!selected.courier} />
                  <DetailField label="Стоимость" value={selected.price ? `${selected.price} ₽` : "—"} />
                </div>

                {selected.items && (
                  <div style={s.detailBlock}>
                    <div style={s.detailBlockLabel}>Состав</div>
                ...
                {selected.comment && (
                  <div style={s.detailBlock}>
                    ...
                    <div style={s.detailBlockValue}>{selected.comment}</div>
                  </div>
                )}

                <div style={s.detailBlock}>
                  <div style={s.detailBlockLabel}>Комментарий оператора</div>
                  <textarea
                    style={s.commentArea}
                    rows={3}
                    value={opComment}
                  ...
                  />
                  <button
                    style={{ ...s.saveCommentBtn, background: commentSaved ? "#1a9e5c" : "#4a7aff" }}
                    disabled={savingComment}
                    onClick={saveComment}
                  >
                    {commentSaved ? "✓ Сохранено" : savingComment ? "..." : "Сохранить"}
                  </button>
                </div>
              </div>
            )}
              ...
              {tableOrders.map((o, i) => {
                ...
                return (
                  <tr
                    key={o.id}
                    style={{
                      background: isSelected ? "#eef3ff" : i % 2 === 0 ? "#fff" : "#fafaf8",
                      cursor: "pointer",
                      outline: isSelected ? "1px solid #4a7aff" : "none",
                    }}
...
function DetailField({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div style={s.detailField}>
      ...
      <div style={{ ...s.detailFieldValue, color: highlight ? "#d94040" : "#1a1a18" }}>{value}</div>
    </div>
  );
}
...
const s: Record<string, React.CSSProperties> = {
  ...
  detailFieldValue: { fontSize: 12, fontWeight: 500 },
