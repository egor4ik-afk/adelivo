...
const STATUS_LABELS: Record<string, string> = {
  NEW: "Новый",
  GEOCODED: "Геокодирован",
  INVALID_ADDRESS: "⚠ Адрес",
  ASSIGNED: "Назначен",
  IN_DELIVERY: "В пути",
  DELIVERED: "Доставлен",
  RETURNED: "Возврат",
...
export function DashboardClient({ user }: { user: User }) {
  ...
  // ── Push: клик по уведомлению из SW ──────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    loadYMaps().then(() => {
      ...
      const map = new window.ymaps.Map(
        mapRef.current,
        { center: [55.752, 37.617], zoom: 10, controls: ["zoomControl"] },
        {}
      );
    ...
    const placemarks = filtered
      ...
      .map((order) => {
        ...
        const pm = new window.ymaps.Placemark(
          [order.lat!, order.lng!],
          {
            ...
            balloonContentBody: `
              <div style="font-size:13px;line-height:1.5">
                ...
                ${order.courier ? `<span>${order.courier}</span><br>` : ""}
          ...
          {
            preset: selectedId === order.id ? "islands#redDotIcon" : "islands#dotIcon",
            iconColor: color,
          }
        );
  ...
  useEffect(() => {
    if (selected?.lat && selected?.lng) {
      ymapRef.current?.panTo([selected.lat, selected.lng], { flying: true });
    }
  }, [selected]);
            ...
            {loading ? (
              ...
              filtered.map((o) => (
                <OrderCard
                  ...
                  onSelect={() =>
                    setSelectedId((p) => (p === o.id ? null : o.id))
                  }
                />
              ))
            )}
            ...
            <DetailBar
              ...
              onSaveComment={async (c) => {
                await fetch(`/api/orders/${selected.id}`, {
                  ...
                  body: JSON.stringify({ opComment: c }),
                });
              }}
            />
          )}
          ...
          {invalid.map((o) => (
            ...
            >
              ...
              <div style={s.alertSub}>
                {o.externalId} · {o.invalidReason ?? "Требует проверки"}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <div style={s.cardMeta}>
        <span style={{ ...s.slotTag, color }}>
          {order.slotFrom}–{order.slotTo ?? ""}
        </span>
        <span style={s.courierTag}>{order.courier ?? "—"}</span>
        {order.price != null && (
          <span style={s.priceTag}>{order.price} ₽</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={s.detailBar}>
      <div style={{ flex: 1 }}>
        ...
        {order.items && (
          <div style={{ fontSize: 12, color: "#6b6860", marginBottom: 3 }}>
            <b style={{ color: "#a8a49c" }}>Состав: </b>
            {order.items}
          </div>
        )}
        {order.comment && (
          <div style={{ fontSize: 12, color: "#6b6860" }}>
            <b style={{ color: "#a8a49c" }}>Клиент: </b>
            {order.comment}
          </div>
        )}
      </div>
      ...
      <div style={{ width: 200, flexShrink: 0 }}>
        ...
        <button
          style={{
            ...
            padding: "6px 12px",
            borderRadius: 6,
            background: saved ? "#1a9e5c" : "#4a7aff",
            border: "none",
            color: "#fff",
            fontSize: 11,
            fontWeight: 600,
