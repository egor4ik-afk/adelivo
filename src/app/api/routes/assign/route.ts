    ...
    for (let i = 0; i < orderIds.length; i++) {
      ...
      if (courierFullName && orderToUpdate?.crmId) {
        ...
        await updateCrmOrder(orderToUpdate.crmId, {
          ...
          // Назначение у нас остаётся в силе, но в CRM его нет — это надо
          // видеть в логах, а не узнавать по слетевшему курьеру
          console.error(
            `[Assign] Курьер ${courierFullName} (id ${courierDb?.id}) не сохранился в CRM ` +
            `для заказа ${orderToUpdate.crmId}:`, e?.response?.data ?? e?.message
          );
        });
      }
    }
