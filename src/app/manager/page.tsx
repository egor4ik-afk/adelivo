          ...
          <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="pt-4 border-t-2 border-dashed border-[#e8e6df]">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h2 className="text-base sm:text-lg font-bold text-[#1a1a18] leading-tight">Ожидают загрузки на базе</h2>
                      <p className="text-[11px] sm:text-sm text-[#a8a49c] font-medium mt-0.5">Назначены, но еще не в пути</p>
                    </div>
                    <div className="ml-auto bg-[#e8e6df] text-[#6b6860] font-black px-2.5 py-0.5 rounded-lg text-sm sm:text-base">{pendingRoutes.length}</div>
                  </div>
                  ...
                  {pendingRoutes.length === 0 && <p className="text-[#a8a49c] font-medium text-center py-8 text-sm">Все назначенные курьеры уже выехали</p>}
                </div>
              </div>
            )}
            ...
            {activeTab === 'routes' && (
              <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-[#e8e6df] shadow-sm mb-2 sticky top-[72px] sm:top-[84px] z-10 gap-3 sm:gap-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <input type="checkbox" className="w-4 h-4 sm:w-5 sm:h-5 accent-[#1a1a18] rounded cursor-pointer shrink-0" checked={isAllGlobalSelected} onChange={toggleAllGlobalOrders} />
                    <span className="font-bold text-[#1a1a18] text-[13px] sm:text-[15px]">Выбрать все {searchQuery ? 'найденные' : ''}</span>
                    {selectedOrders.size > 0 && <span className="bg-[#eef3ff] text-[#4a7aff] px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md sm:rounded-lg text-[11px] sm:text-sm font-bold border border-[#dce6ff] ml-1 sm:ml-2 shadow-sm">Выбрано: {selectedOrders.size}</span>}
                  </div>
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <button onClick={handlePrintLabels} disabled={selectedOrders.size === 0} className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-1 sm:gap-2 ${selectedOrders.size > 0 ? 'bg-[#1a1a18] text-white hover:bg-gray-800' : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'}`}>
                      ...
                      <div key={route.id} className="bg-white border border-[#e8e6df] rounded-xl shadow-sm transition-all overflow-hidden">
                        <div className="flex flex-row items-center justify-between p-3 hover:bg-[#fafaf8] cursor-pointer transition-colors gap-2" onClick={() => toggleRouteExpansion(route.id)}>
                          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap overflow-hidden">
                            ...
                            <span className="text-[#a8a49c] w-3 text-center text-[10px] shrink-0">{isExpanded ? '▼' : '▶'}</span>
                            <span className="font-black text-[13px] sm:text-[14px] text-[#1a1a18] whitespace-nowrap">{route.courier?.fullName || 'Не назначен'}</span>
                            <span className="text-[10px] font-extrabold text-[#6b6860] bg-[#f5f4f0] px-1.5 py-0.5 rounded-md border border-[#e8e6df] whitespace-nowrap shadow-sm">🗺️ {route.name || `#${route.id.slice(-5).toUpperCase()}`}</span>

                            {courierPhone !== "—" && (
                              <div className="flex items-center gap-1.5 ml-1">
                                <a href={`tel:${courierPhone}`} onClick={e => e.stopPropagation()} className="text-[#4a7aff] font-semibold text-[11px] sm:text-[13px] hover:underline whitespace-nowrap">
                                  📞 {courierPhone}
                                </a>
                                {cleanPhoneForTg && (
                                  <>
                            ...
                            {route.plannedDepartureTime && <span className="text-[10px] font-bold text-[#1a1a18] bg-[#f5f4f0] px-1.5 py-0.5 rounded-md border border-[#e8e6df] shadow-sm">🏠 {route.plannedDepartureTime}</span>}
                            ...
                            <span className="text-[10px] font-bold text-[#6b6860] bg-[#f5f4f0] px-1.5 py-0.5 rounded-md border border-[#e8e6df] whitespace-nowrap hidden sm:inline-block">Точек: {route.orders?.length || 0}</span>
                          </div>
                        ...
                        {isExpanded && (
                          <div className="p-3 sm:p-4 bg-[#f5f4f0] border-t border-[#e8e6df]">
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
                              {(() => {
                                ...
                                return sortedOrders.map((order: any, idx: number) => {
                                  ...
                                  return (
                                    <div key={order.id} className="flex flex-col bg-white rounded-xl border border-[#e8e6df] shadow-sm p-2.5 sm:p-3 relative hover:border-[#dcd9d1] transition-colors">
                                      <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center gap-2">
                                          ...
                                          <span className="font-black text-[13px] sm:text-[14px] text-[#1a1a18] truncate max-w-[100px]" title={String(displayId)}>{displayId}</span>
                                        </div>
                                      ...
                                      <div className="flex flex-col gap-1 mt-1 mb-2">
                                        ...
                                        {order.comment && (
                                          <div className="bg-[#fff1f2] border border-[#ffe4e6] p-1.5 rounded">
                                            <p className="text-[10px] sm:text-[11px] text-[#881337]"><span className="font-bold text-[#be123c] uppercase tracking-wider text-[8px] sm:text-[9px] block mb-0.5">Коммент заказчика</span> {order.comment}</p>
                                          </div>
                                        )}
                                        ...
                                        {order.opComment && (
                                          <div className="bg-[#f0fdf4] border border-[#dcfce7] p-1.5 rounded mt-1">
                                            <p className="text-[10px] sm:text-[11px] text-[#14532d]"><span className="font-bold text-[#15803d] uppercase tracking-wider text-[8px] sm:text-[9px] block mb-0.5">Оператор</span> {order.opComment}</p>
                                          </div>
                                        )}
                                      </div>
            ...
            {activeTab === 'history' && (
              <>
                {history.map((task) => (
                  <div key={task.id} className="bg-white border border-[#e8e6df] rounded-xl p-4 shadow-sm hover:border-[#dcd9d1] transition-colors flex flex-col md:flex-row justify-between items-start gap-4">
                    <div className="flex flex-col gap-1 w-full md:w-1/3 shrink-0">
                      <div>
                        <p className="text-[15px] font-extrabold text-[#1a1a18]">{task.courierName}</p>
                        {task.routeName && <div className="text-[11px] font-medium text-[#a8a49c] mt-0.5">маршрут {task.routeName}</div>}
                      </div>
                      <div className="mt-2">
                        <p className="text-[11px] sm:text-[12px] font-medium text-[#878378]">Изменил: <span className="font-bold text-[#1a1a18]">{task.authorName || 'Система'}</span></p>
                        <p className="text-[11px] sm:text-[12px] font-medium text-[#a8a49c] mt-0.5">{new Date(task.createdAt).toLocaleString('ru-RU', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-2/3 md:items-end">
                      ...
                      <div className="bg-[#faf9f7] rounded-lg p-3 border border-[#e8e6df] w-full text-left mt-1">
                        {task.oldValue && (
                          <div className="mb-2">
                            <span className="text-[9px] font-bold text-[#a8a49c] uppercase tracking-widest block mb-0.5">Было:</span>
                            <span className="text-[12px] sm:text-[13px] font-medium text-[#878378] line-through break-words whitespace-pre-wrap">{task.oldValue}</span>
                          </div>
                        )}
                        <div>
                          <span className="text-[9px] font-bold text-[#a8a49c] uppercase tracking-widest block mb-0.5">Стало:</span>
                          <span className="text-[13px] sm:text-[14px] font-extrabold text-[#1a1a18] break-words whitespace-pre-wrap">{task.newValue}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ))}
                {history.length === 0 && <p className="text-[#a8a49c] text-center py-10 text-sm">История пуста</p>}
              </>
            )}
          </div>
        )}
