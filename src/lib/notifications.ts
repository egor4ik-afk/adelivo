...
// Инициализация только в runtime, не при импорте модуля
...
export async function notify(event: NotificationEvent) {
  switch (event.type) {
    ...
    case "address.invalid": {
      ...
      // Push для каждого кривого адреса отдельно — чтобы кликнуть и открыть нужный
      for (const o of orders) {
        try {
          await pushToAllOperators(
            `⚠ Проблемный адрес: ${o.externalId ?? "—"}`,
            `${o.address ?? "—"} — ${o.reason}`,
