  ...
  const payload = {
    ...
    duties: [
      { 
        ...
        description: "Услуга по доставке заказов, оформленных через мобильное приложение заказчика, по адресам, указанным заказчиком, в административных границах города Москва или ближайшего Подмосковья. Исполнитель получает информацию о заказе, включая конечный адрес и контактные данные получателя, через мобильное приложение или электронную почту. Исполнитель самостоятельно забирает заказ из точки выдачи и доставляет его получателю, сверяя адрес и контактные данные на месте вручения. Результат оказания услуги: 1 (один) доставленный заказ по точному адресу, полученному от заказчика, с вручением заказа получателю и подтверждением получения заказа",
        price: baseAmount, 
        quantity: 1
      }
    ],
