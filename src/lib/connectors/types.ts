...
// Тип живёт в catalog.ts (чистые данные, безопасен для клиента).
// Здесь только реэкспорт — иначе `export *` из index.ts выдаст конфликт:
// одно и то же имя пришло бы из двух разных объявлений.
